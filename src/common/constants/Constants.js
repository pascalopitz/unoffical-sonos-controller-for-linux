const Constants = {
    STORE_RESET: '',

    VOLUME_CONTROLS_MUTE_SET: '',
    VOLUME_CONTROLS_VOLUME_SET: '',
    VOLUME_CONTROLS_DRAGGING: '',
    VOLUME_CONTROLS_EXPANDED: '',
    VOLUME_CONTROLS_QUERY_VOLUMES: '',

    BROWSER_SELECT_ITEM: '',
    BROWSER_SCROLL_POSITION: '',
    BROWSER_SCROLL_RESULT: '',
    BROWSER_SEARCH: '',
    BROWSER_SEARCH_EXIT: '',
    BROWSER_BACK: '',
    BROWSER_HOME: '',
    BROWSER_PLAY: '',
    BROWSER_PLAY_NEXT: '',
    BROWSER_ADD_QUEUE: '',
    BROWSER_REPLACE_QUEUE: '',
    BROWSER_ADD_MUSICSERVICE: '',
    BROWSER_REMOVE_MUSICSERVICE: '',
    BROWSER_ADD_TO_PLAYLIST: '',
    BROWSER_EDIT_PLAYLIST: '',
    BROWSER_DELETE_PLAYLIST: '',
    BROWSER_DELETE_FAVOURITE: '',

    PLAYER_PAUSE: '',
    PLAYER_PLAY: '',
    PLAYER_PREV: '',
    PLAYER_NEXT: '',
    PLAYER_SEEK: '',

    QUEUE_GOTO: '',
    QUEUE_FLUSH: '',
    QUEUE_REMOVE: '',
    QUEUE_REMOVE_SELECTED: '',
    QUEUE_SELECT: '',
    QUEUE_DESELECT: '',
    QUEUE_REORDER: '',
    QUEUE_SAVE: '',

    ZONE_GROUP_SELECT: '',

    EQ_SHOW: '',
    EQ_HIDE: '',
    EQ_LOAD: '',
    EQ_SET_VALUE: '',
    EQ_SELECT: '',
    EQ_BREAK_PAIR: '',
    EQ_CREATE_PAIR: '',
    EQ_TRUEPLAY: '',

    PLAYLISTS_SHOW: '',
    PLAYLISTS_HIDE: '',
    PLAYLISTS_LOAD: '',
    PLAYLISTS_ITEMS_LOAD: '',
    PLAYLISTS_TOGGLE: '',
    PLAYLISTS_ADD_ITEM: '',
    PLAYLISTS_MOVE_ITEM: '',
    PLAYLISTS_DELETE_ITEM: '',
    PLAYLISTS_SAVE: '',

    GROUP_MANAGEMENT_HIDE: '',
    GROUP_MANAGEMENT_SHOW: '',
    GROUP_MANAGEMENT_TOGGLE: '',
    GROUP_MANAGEMENT_SAVE: '',

    SONOS_SERVICE_LOCAL_FILES_STATE: '',
    SONOS_SERVICE_DEVICE_SEARCH_RESULT: '',
    SONOS_SERVICE_LIBRARY_INDEX_UPDATE: '',
    SONOS_SERVICE_TOPOLOGY_UPDATE: '',
    SONOS_SERVICE_ZONES_UPDATE: '',
    SONOS_SERVICE_ZONEGROUP_TRACK_UPDATE: '',
    SONOS_SERVICE_ZONEGROUPS_DEFAULT: '',
    SONOS_SERVICE_QUEUE_UPDATE: '',
    SONOS_SERVICE_VOLUME_UPDATE: '',
    SONOS_SERVICE_MUTED_UPDATE: '',
    SONOS_SERVICE_PLAYSTATE_UPDATE: '',
    SONOS_SERVICE_NEXT_TRACK_UPDATE: '',
    SONOS_SERVICE_POSITION_INFO_UPDATE: '',
    SONOS_SERVICE_MEDIA_INFO_UPDATE: '',
    SONOS_SERVICE_RENDERING_CONTROL_UPDATE: '',
    SONOS_SERVICE_CURRENT_PLAY_MODE_UPDATE: '',
    SONOS_SERVICE_CURRENT_CROSSFADE_MODE_UPDATE: '',
    SONOS_SERVICE_MUSICSERVICES_UPDATE: '',
    SONOS_SERVICE_REFRESHPOSITION: '',
    SONOS_SERVICE_WAKEUP: '',

    OPTIMISTIC_CURRENT_PLAY_MODE_UPDATE: '',
    OPTIMISTIC_CURRENT_CROSSFADE_MODE_UPDATE: '',

    MUSICSERVICE_ADD_CANCEL: '',
    MUSICSERVICE_ADD_LINK_RECEIVED: '',
    MUSICSERVICE_AUTH_TOKEN_RECEIVED: '',
    MUSICSERVICE_SESSION_ID_RECEIVED: '',
    MUSICSERVICE_ANONYMOUS: '',

    CURRENT_TRACK_TOGGLE_EXPANDED: '',
};

Object.keys(Constants).forEach((k) => {
    Constants[k] = Symbol(k);
});

export default Constants;
