...
class MusicServiceClient {
            ...
            requestHelper({
                ...
                if(err || res.statusCode >= 400 || fault) {
                    ...
                    if(fault.indexOf('TokenRefreshRequired') > -1) {
                        const refreshDetails = _.get(e, 'Envelope.Body.Fault.detail.refreshAuthTokenResult');
    ...
    getDeviceLinkCode() {
        ...
        return this._doRequest(this._serviceDefinition.SecureUri, 'getDeviceLinkCode', body, headers)
            .then((res) => {
                ...
                const obj = resp['Envelope']['Body']['getDeviceLinkCodeResponse']['getDeviceLinkCodeResult'];
                return obj;
            });
    }
    ...
    getAppLink() {
        ...
        return this._doRequest(this._serviceDefinition.SecureUri, 'getAppLink', body, headers)
            .then((res) => {
                const resp = xml2json(stripNamespaces(res));
                const obj = resp['Envelope']['Body']['getAppLinkResponse']['getAppLinkResult'];
    ...
    getDeviceAuthToken(linkCode, linkDeviceId) {
        ...
        const body = ['<ns:getDeviceAuthToken>',
            '<ns:householdId>', SonosService.householdId, '</ns:householdId>',
        ...
        return this._doRequest(this._serviceDefinition.SecureUri, 'getDeviceAuthToken', body, headers)
            .then((res) => {
                const resp = xml2json(stripNamespaces(res));
                const obj = resp['Envelope']['Body']['getDeviceAuthTokenResponse']['getDeviceAuthTokenResult'];
                return obj;
            });
    }
    ...
    getMetadata(id, index=0, count=200) {
        ...
        return new Promise((resolve, reject) => {
            this._doRequest(this._serviceDefinition.SecureUri, 'getMetadata', body, headers)
                .then((res) => {
                    const resp = xml2json(stripNamespaces(res));
                    const obj = resp['Envelope']['Body']['getMetadataResponse']['getMetadataResult'];
                    resolve(obj);
                })
                .catch((response) => {
    ...
    getExtendedMetadata(id) {
        ...
        const body = ['<ns:getExtendedMetadata>',
            '<ns:id>', id, '</ns:id>',
        ...
        return new Promise((resolve, reject) => {
            this._doRequest(this._serviceDefinition.SecureUri, 'getExtendedMetadata', body, headers)
                .then((res) => {
                    const resp = xml2json(stripNamespaces(res));
                    const obj = resp['Envelope']['Body']['getExtendedMetadataResponse']['getExtendedMetadataResult'];
                    resolve(obj);
                })
                .catch((authToken) => {
                    if(authToken) {
    ...
    search(id, term, index=0, count=200) {
        ...
        return new Promise((resolve, reject) => {
            return this._doRequest(this._serviceDefinition.SecureUri, 'search', body, headers)
                .then((res) => {
                    const resp = xml2json(stripNamespaces(res));
                    const obj = resp['Envelope']['Body']['searchResponse']['searchResult'];
                    resolve(obj);
                })
                .catch((authToken) => {
                    if(authToken) {
    ...
    getMediaURI(id) {
        ...
        return new Promise((resolve, reject) => {
            return this._doRequest(this._serviceDefinition.SecureUri, 'getMediaURI', body, headers)
                .then((res) => {
                    const resp = xml2json(stripNamespaces(res));
                    const obj = resp['Envelope']['Body']['getMediaURIResponse']['getMediaURIResult'];
                    return resolve(obj);
                })
                .catch((authToken) => {
                    if(authToken) {
    ...
    getSessionId(username, password) {
        ...
        return this._doRequest(this._serviceDefinition.SecureUri, 'getSessionId', body, headers)
            .then((res) => {
                const resp = xml2json(stripNamespaces(res));
                const obj = resp['Envelope']['Body']['getSessionIdResponse']['getSessionIdResult'];
                return obj;
            });
    }
