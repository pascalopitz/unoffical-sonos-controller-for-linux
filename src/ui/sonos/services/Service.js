...
class Service {
    ...
    _request (action, variables, callback) {
        const message_action = '"urn:schemas-upnp-org:service:' + this.name + ':1#' + action + '"';
        const message_body_pre = '<u:' + action + ' xmlns:u="urn:schemas-upnp-org:service:' + this.name + ':1">';
        const message_body_post = '</u:' + action + '>';
        const message_body = message_body_pre + _.map(variables, function(value, key) {
