...
export default {
    ...
    _getItem (item) {
        ...
        const settingsMatch = _.find(SonosService._accountInfo, { Type: String(serviceType) } );

        if(settingsMatch) {
            const uri = client.getTrackURI(item.id, client._serviceDefinition.Id, settingsMatch.SerialNum);
            const token = client.getServiceString(serviceType, settingsMatch.Username);
